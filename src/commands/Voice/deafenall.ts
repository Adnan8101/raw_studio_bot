...
export const data = new SlashCommandBuilder()
    .setName('deafenall')
    .setDescription('Deafen everyone in your voice channel')
...
export async function execute(interaction: ChatInputCommandInteraction) {
    ...
    for (const [_, m] of members) {
        try {
            ...
            console.error(`Failed to deafen ${m.user.tag}:`, e);
        }
    }

    await interaction.editReply({ embeds: [createSuccessEmbed(`Deafened **${count}** members in ${channel}.`)] });
}
